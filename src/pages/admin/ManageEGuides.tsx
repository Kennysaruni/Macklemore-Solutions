...
export default function ManageEGuides() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (file) {
        ...
        const filePath = `eguides/${fileName}`;
      ...
      const { error } = await supabase.from('eguides').insert([
      ...
      console.error('Error adding e-guide:', error);
      alert('Error adding e-guide. Check console.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string, imageUrl: string) => {
    if (!confirm('Are you sure you want to delete this e-guide?')) return;
    
    setLoading(true);
    try {
      const { error: dbError } = await supabase.from('eguides').delete().eq('id', id);
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <h2 className="text-xl font-display font-semibold text-slate-900 mb-6">Add E-Guide / Whitepaper</h2>
        <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-brand-blue focus:border-brand-blue"
              placeholder="Write the full e-guide content or an executive summary to be read on the page..."
            ></textarea>
          </div>
          <div>
          ...
          >
            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Plus className="w-5 h-5 mr-2" /> Add E-Guide</>}
          </button>
        </form>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200">
          <h2 className="text-xl font-display font-semibold text-slate-900">Current E-Guides</h2>
        </div>
        <ul className="divide-y divide-slate-200">
          ...
          {eguides.length === 0 && (
            <li className="p-6 text-center text-slate-500">No e-guides found.</li>
          )}
        </ul>
      </div>
    </div>
